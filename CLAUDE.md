...
# NAV2 GOAL RELIABILITY (plan Phase 7): reproduce/measure goal outcomes with a per-goal time series (no code changes)
python3 tests/sim/experiments/nav_goal_diag.py --trials 20 --reinit --goals G1,G2,G3        # maps once, then AMCL+Nav2; --map <yaml> reuses a saved map
python3 tests/sim/experiments/nav_goal_diag.py --probe 24 --map <yaml>                        # how far does the planner's path END from the goal?
python3 tests/sim/experiments/map_precision_diag.py --runs 8                                   # map precision raw vs after removing the rigid offset
...
python3 -m pytest tests -q -rPx                       # EVERYTHING (403 tests: 255 unit, 48 node, 100 sim): ~30-35 min on a healthy host, far more on a stalling one (see "Test reliability"); -rPx prints the MEASURED values
python3 -m pytest tests/sim/test_robot.py -k spin -v  # one module / one case
```

**Test reliability on this host (measured, plan Phases 6–8).** A complete run takes ~30 min on a healthy host (Phase 5: 217 passed + 1 xfail; later runs: 372 passed, 1 xfail, a few intermittent failures; plan Phase 7 final: **401 passed, 2 xfailed, 0 failed in 34 min**, 303 non-sim + 100 sim tests). **The Nav2 detour-goal failure is fixed** (plan Phase 7, controller latch; see "Navigation"). **The map-precision test is also fixed** (plan Phase 7, maintainer-approved): `test_occupied_cells_lie_on_real_surfaces` was measuring a ~4 cm rigid map offset from Gazebo's frame as much as map shape (raw 68–93 % against a 70 % threshold, 87–98.5 % after removing the offset, over 11 maps), so it now grid-searches the best-fit rigid translation and scores the aligned distances against the SAME 70 %/95 % thresholds, plus a new hard bound that the offset itself stays ≤ 10 cm. One Phase 4 test remains **intermittent, environmental, deliberately not loosened**: `test_scan_rate` (the depth/`/scan` render rate is 11–28 Hz per sim second on every tree, floor 12 Hz; `sensor_rate_diag.py`). A **stalling host** (Windows-side; seen after long uptime; a WSL reboot cleared it) makes far more fail: robot motion, topic/scan rates, fixture start-up, an 8.5-min build instead of 17 s. So: before trusting a sim failure, check the real-time factor (`gz topic -e -t /stats -n 6` → `real_time_factor` ≈ 1.0, ignoring the first samples, and no 0.5 / 3.0 swings), rerun the module, and if it still fails A/B it against a clean worktree of the last accepted commit (`git worktree add <dir> <commit>` + `colcon build`, alternating runs, *several* of each: single runs mislead, and the tally of a tree that was mostly run inside full-suite runs was confounded). To isolate SLAM/map randomness from a Nav2 result, reuse one saved map for both trees: the mapping fixture saves it under `/tmp/fire_resq_map_*/arena.yaml`; then `FIRE_RESQ_TEST_MAP=<that yaml> python3 -m pytest tests/sim/test_nav2.py`. Tests that count messages must count per *simulation* second, never wall-clock; `lifecycle_state()` returns `unknown` on a slow `ros2` CLI call so wait loops keep polling.
...
**Goal tolerance and the controller latch (plan Phase 7, measured — read before touching the controller or goal checker).** The Regulated Pure Pursuit controller takes its arrival radius from the goal checker's `xy_goal_tolerance` (they are the same number by construction) and, with its own `stateful` parameter on (the default), **latches** "arrived" the first cycle it is inside and then only rotates in place. The goal checker needs its own evaluation to see the robot inside the same radius a cycle (and an AMCL update) later; when it is a fraction of a centimetre outside, the robot froze there until the progress checker aborted: 44 % of G1 and 12 % of the detour goal G2 goals failed (fresh AMCL each trial), 0 of 60 after `FollowPath.stateful: false`. **Keep it false**; the goal *checker's* `stateful: true` is the correct one. Consequences: goals still end at the tolerance edge (truth 6–14 cm from the goal), so the tests' 25 cm tolerance is the accuracy actually available, and a robot that must be centimetre-accurate (the magnet) cannot rely on Nav2's goal. Also: the planner's `tolerance: 0.2` used to report "reachable" for goals inside thin obstacles; `Nav2PathQuery` now requires the path to END within one cell of the goal. And **unknown space is plannable** (`allow_unknown: true`): a goal inside the never-seen interior of a block comes back reachable (recorded `xfail`).

**Experiments that lie.** Changing controller parameters at run time (`ros2 param set`) in a long AMCL session is a trap: one variant that strands the robot leaves AMCL initialised at the map origin while the robot is elsewhere, and every later trial aborts within a second. Reset the robot (or restart the stack) between variants, and never draw conclusions from trials where the robot did not move.
...
- **Planner questions are cached (reachable answers only, 15 s, matched to 5 cm)**; `unreachable`/`unknown` are never cached and a retry/refresh bypasses the cache. A path only counts as reachable if it ENDS at the goal. `SelectTarget` on an unchanged world costs the planner nothing. A missing planner is noticed once (then `unknown` for 2 s), not once per question.
- **The approach pose contract for Phase 10:** `RescueTarget.approach_pose` is 0.4 m from the victim's centre on the robot→victim line (`factors.approach_pose`, `map` frame), facing the victim. Nav2 can plan no closer than 0.265 m and magnet contact is at 0.131 m (both derived from the robot description and the victim model, unit-guarded), so ~0.27 m of `ALIGN` creep is Phase 10's and open-loop.
...
**Next up: plan Phase 9** — the electromagnet in simulation (a detachable joint, `ATTACH`/`RELEASE`, `MagnetState`; the robot-side `magnet_link` and the victims' steel rings already exist), then Phase 10 (the rescue loop: it calls `SelectTarget`, drives Nav2 to `approach_pose`, then does the open-loop `ALIGN` creep the planner cannot). Navigate under AMCL on a saved map, not concurrently with scan-matching SLAM. Phases 0–7 and cognition are done; [docs/Implementation_Plan.md](docs/Implementation_Plan.md) §13 defines each phase's scope, verification, and definition of done. Do not implement ahead of the current phase — the TODO markers in each package mark where later work attaches.
